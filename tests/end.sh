...
tap_cmp_str "$out"  "1..2" "we used two tests before this tap_end"
tap_cmp_int 0 $end_rc "since every test succeeded: resultcode of tap_end must be 0"
...
end_rc=$?
out="`echo "$end_out" | grep -v "^#"  | head -1`"
tap_cmp_str "$out"  "1..42" "we forced 42 as test counter in this tap_end"
tap_cmp_int 1 $end_rc "we forced a different test counter: resultcode of tap_end must be 1"


# we need a failed test to make sure, that tap_end returns failure
echo -n "ok 7 - overwrite result: (" && tap_fail "expecting a 'not ok' here)"

# verify plan line: "1..7" and resultcode: 1 (we forced a failing test)
end_out=$( tap_end )
end_rc=$?
out="`tap_end | grep -v "^#" | head -1 `"
tap_cmp_str "$out"  "1..7" "we used 7 tests before this tap_end"
tap_cmp_int 1 $end_rc "we forced a failing test: resultcode of tap_end must be 1"
